...
void printStatus(){
  ...
  Serial.println("========== Ather OBD Live ==========");
  Serial.printf(" SoC: %6.2f %% | SoH: %6.2f %% | Delta: %6.2f %%\n", soc, soh, deltaSoc);
  Serial.printf(" Voltage: %6.2f V | Imbalance: %.4f V | Balancing: %d\n", voltage, imbalance, balancing);
  Serial.printf(" RPM: %5d | Drive: %-8s | Range: %d km (CAN 0x200)\n", rpm, driveModeName(driveMode), rangeKm);
  Serial.printf(" Frames: %lu | dropped:%lu | bus:%d\n", (unsigned long)framesTotal, (unsigned long)st.rx_missed_count, st.state);
  Serial.println("-------- Switches --------");
  Serial.printf(" Key:%-8s Stand:%-4s Start:%-11s\n", keyOn?"Enabled":"Disabled", sideStand?"DOWN":"UP", startSwitch?"Pressed":"Not Pressed");
  Serial.printf(" F-Brake:%-11s R-Brake:%-11s H-Beam:%-11s\n", frontBrake?"Pressed":"Not Pressed", rearBrake?"Pressed":"Not Pressed", highBeam?"Pressed":"Not Pressed");
  Serial.printf(" Horn:%-11s Kill:%-11s Storage:%-11s\n", horn?"Pressed":"Not Pressed", killSwitch?"Pressed":"Not Pressed", storageSwitch?"Pressed":"Not Pressed");
  Serial.printf(" Ind L:%-11s R:%-11s C:%-11s\n", indicatorLeftSide?"Pressed":"Not Pressed", indicatorRightSide?"Pressed":"Not Pressed", indicatorCenterSide?"Pressed":"Not Pressed");
}

void setup(){
  Serial.begin(115200); delay(1000);
  Serial.println("\n=== Ather OBD Scanner (simple) ===");
  ...
  Serial.printf("AP %s! SSID:%s IP:%s BSSID:%s TX=%.1fdBm\n", apOk?"Started":"FAILED", AP_SSID, ip.toString().c_str(), WiFi.softAPmacAddress().c_str(), 0.25*WiFi.getTxPower());
  server.on("/",handleRoot); server.on("/data",handleData); server.begin();
  Serial.println("Web server http://192.168.4.1/ (updates 1s)");
  setupCAN();
}
void loop(){
  server.handleClient();
